...
/** Paint a parallax layer once, then scroll it by blitting it repeatedly. */
export function buildBackdrop(theme: Theme, width: number, height: number): HTMLCanvasElement {
  const canvas = document.createElement('canvas');
  canvas.width = width;
  ...
  // Far layer: soft rolling shapes along the bottom.
  ctx.fillStyle = theme.far;
  for (let x = -20; x < width + 20; x += 60) {
    const bump = 26 + ((x * 7) % 18);
    ctx.beginPath();
    ctx.ellipse(x, height, 52, bump, 0, Math.PI, 0);
    ctx.fill();
  }
  ...
  switch (theme.decor) {
    case 'tree':
      for (let x = 10; x < width; x += 74) {
        ctx.fillRect(x + 8, height - 26, 5, 26);
        ctx.beginPath();
        ctx.ellipse(x + 10, height - 32, 18, 16, 0, 0, Math.PI * 2);
        ctx.fill();
      }
      break;
    case 'peak':
      for (let x = -10; x < width + 40; x += 86) {
        ctx.beginPath();
        ctx.moveTo(x, height);
        ctx.lineTo(x + 34, height - 52);
        ctx.lineTo(x + 68, height);
        ctx.closePath();
        ctx.fill();
      }
      break;
    case 'palm':
      for (let x = 20; x < width; x += 92) {
        ctx.fillRect(x + 9, height - 34, 4, 34);
        for (let frond = -2; frond <= 2; frond++) {
          ctx.beginPath();
          ctx.ellipse(x + 11 + frond * 9, height - 36, 11, 4, frond * 0.4, 0, Math.PI * 2);
          ctx.fill();
        }
      }
      break;
